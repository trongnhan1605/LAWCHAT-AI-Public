...
class ProvisionRelation(Base):
    ...
    source_provision_id: Mapped[int] = mapped_column(ForeignKey("legal_provisions.id"), nullable=False, index=True)
    target_document_id: Mapped[int] = mapped_column(ForeignKey("documents.id"), nullable=False, index=True)
    target_provision_id: Mapped[int] = mapped_column(ForeignKey("legal_provisions.id"), nullable=False, index=True)
    ...
    source_excerpt: Mapped[str | None] = mapped_column(UnicodeText, nullable=True)
    target_excerpt: Mapped[str | None] = mapped_column(UnicodeText, nullable=True)
    confidence_score: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
    extraction_method: Mapped[str | None] = mapped_column(Unicode(32), nullable=True)
